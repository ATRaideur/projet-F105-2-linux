...
int chargerTexteBis(const char* cheminMessage, unsigned char* texte, int longueur) {
    ifstream f(cheminMessage);
    ...
    if (f) {
        ...
        buffer << f.rdbuf();
        string tmp = buffer.str();
        int len = min(longueur, static_cast<int>(tmp.size())); // calculer la longueur à copier
        memcpy(texte, tmp.c_str(), len); // copier le texte
        return 0;
    }
    else {
        fprintf(stderr, "Fichier %s ne peut pas être ouvert\n", cheminMessage);
...
int chargerTexteTers(const char* cheminMessage, char* texte) {
    ...
    if (f) {
        ...
        buffer << f.rdbuf();
        string tmp = buffer.str();
        strcpy(reinterpret_cast<char*>(texte), tmp.c_str());
        return 0;
    }
    else {
        fprintf(stderr, "Fichier %s ne peut pas être ouvert\n", cheminMessage);
...
int sauverFichierTexteBis(const char* cheminTexte, unsigned char* texte, int longeur) {

    ofstream out(cheminTexte);
    if (out) {
        out.write(reinterpret_cast<const char*>(texte), longeur);        out.close();
        return 0;
    }
    else {
        fprintf(stderr, "Fichier %s ne peut pas être ouvert\n",
