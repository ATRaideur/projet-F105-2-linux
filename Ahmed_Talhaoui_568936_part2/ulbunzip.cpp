...
void decoderMessage(int parent[MAX_NOEUDS], 
	...
	int bitIndexe;
	for(bitIndexe=0; bitIndexe <= MAX_SIZE * 8 + 200; bitIndexe = bitIndexe +1){
...
int main(int argc, char *argv[]) {
	...
	chargerTexteBis(cheminMessageEncode,messageEncode, 1130);
	for(int u = 0 ; u < 1000 ; u ++){

		cout << messageEncode[u] << endl ;
